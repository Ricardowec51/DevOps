...
- **Remoto:** los scripts de la copia activa **sí** están versionados desde 2026-09-01 en `github.com/Ricardowec51/DevOps` (repo del home): `pulso_curator.py`, `pulso_publisher.py`, `pulso_cron.sh`, `pulso_notify_fail.py`, `enviar_edicion_manual.py`. `config.yaml` sigue gitignored (keys en claro). El repo viejo `Ricardowec51/pulso-ia` corresponde a `Downloads/pulso-final` y está obsoleto.
- **Automatización:** crontab de macOS — lunes 8:00am genera, 9:00am reintenta (`pulso_cron.sh`). El pipeline: preflight `codex login status` → curación → `.docx` → copia a `/Volumes/Externo/PULSO_a_la_IA/documentos/` → correo a `ricardowec@gmail.com` con el `.docx` adjunto y las rutas local + disco externo.
- **Descripción:** Newsletter semanal de IA de EMPRENDEDORES.LTD para ejecutivos de **todos los sectores de la economía** en LATAM. Pipeline en Python.
- **Curación (cambió 2026-09-01):** de Google Gemini → **Codex CLI** (`codex exec -m gpt-5.6-luna`). Auth por OAuth del **plan ChatGPT** de Ricardo en `~/.codex/auth.json` (auto-refresh) — NO usa API key. Motivo del cambio: Gemini devolvía 503 los lunes (ediciones 33 y 34). Hubo un intento intermedio con Qwen local vía Ollama, descartado por alucinaciones. La sección "Modelos Destacados" del prompt ahora solo admite modelos nombrados textualmente en los artículos (0–2) para que no invente nombres.
- **Aviso por fallo:** `pulso_notify_fail.py` acepta un motivo; con `codex-no-session` el correo del intento de las 9:00 (`--notify`) explica que hay que correr `codex login`.
- **Bug corregido 2026-08-04:** `pulso_cron.sh` solo miraba si el `.docx` ya existía para decidir si reintentar — un fallo de SMTP en el intento de las 8am hacía que el reintento de las 9am no hiciera nada, y la Edición 31 nunca se envió. Se corrigió con el marcador `cache/edition_N.sent` (solo tras éxito real) y `--send-only` para reintentar solo el envío. Ver memoria `project_pulso_ia.md`.
- **Nota de seguridad:** `.../pulso-ia-gemini/config.yaml` tiene en texto plano: `anthropic_api_key` (ya no se usa), `gemini_api_key` (comentada), `email.smtp_password` (Gmail App Password, en uso) y `facebook.access_token` (en uso si se publica a FB). Está en `.gitignore`, nunca se comiteó. Codex no aporta ninguna key a ese archivo.
- **Estado:** Activo y automatizado. Cambios de 2026-09-01 pusheados (`04b2c2c`..`3d183ea`); la Edición 35 se regeneró y reenvió ese día con el texto y el curador nuevos.
...
- **Python** — Conda/Miniconda3, Ollama, scripts de PULSO a la IA (curación vía Codex CLI, `codex exec -m gpt-5.6-luna`, auth del plan ChatGPT)
...
4. **PULSO a la IA:** migración 2026-09-01 completada y pusheada (`04b2c2c`..`3d183ea` en `Ricardowec51/DevOps`): curación Gemini→Codex CLI (`gpt-5.6-luna`, plan ChatGPT), audiencia ampliada a "todos los sectores de la economía", "Modelos Destacados" ya no inventa nombres, correo de los lunes con `.docx` adjunto + copia a disco externo, y preflight `codex login status` en el cron con aviso por correo. Pendiente menor: si `~/.codex/auth.json` pierde sesión, el cron avisa pero hay que correr `codex login` a mano.
